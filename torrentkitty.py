...
reuse the code to gen the challange answer instead of install node 
...
def try_selenium_request():
    '''
    not usable
    :return:
    '''
    driver = Chrome(chrome_options=chrome_options, executable_path=chrome_driver, desired_capabilities=d)
    ...
    print(driver.get_cookies())
    print('-------------------------------')
    response = driver.request('GET', 'https://torrentkitty.tv/search/')
...
def try_get_log():
    '''
        try to use get_log in driver, but not usable in this case
    :return:
    '''
